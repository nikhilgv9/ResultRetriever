...
public class SpreadSheetCreator {
    ...
    public void createXLS(String fileName,boolean userJxl){
        try {
            ...
            Map<String, Integer> failCount = results.getFailCount();
            
            for(String subCode:results.subjects){
                currentColumn=subjectToColoumnNoMap.get(subCode);
                Integer passcount = passCount.get(subCode);
                if(passcount==null){
                    passcount=new Integer(0);
                }
                Integer failcount = failCount.get(subCode);
                if(failcount==null){
                    failcount=new Integer(0);
                }
                
                double passPer=((double)passcount/(double)(passcount+failcount))*100.0;
            ...
            sheet1.addCell(label); 
            number = new jxl.write.Number(2, currentRow, results.getTotalPass(),normalfontformat);
            sheet1.addCell(number); 
            currentRow++;
            ...
            sheet1.addCell(label); 
             number = new jxl.write.Number(2, currentRow, results.getTotalFail(),normalfontformat);
            sheet1.addCell(number);
            currentRow++;
